...
test("register, encrypt, relogin, decrypt, edit and sync another session",async({page,browser})=>{
 ...
 const syncedItem=page.getByText("Fake demo login",{exact:true});const syncError=await Promise.race([syncedItem.waitFor({timeout:30000}).then(()=>null),authError.waitFor({timeout:30000}).then(()=>authError.textContent())]);if(syncError)throw new Error(`Fresh-session sync failed: ${syncError}`);
 await syncedItem.click();await page.getByRole("button",{name:"Toggle password visibility"}).click();await expect(page.getByText(secret,{exact:true})).toBeVisible();
