...
async function hkdf(root: Uint8Array, label: string) {
  const key = await crypto.subtle.importKey("raw", buffer(root), "HKDF", false, ["deriveBits"]);
    ...
    await crypto.subtle.deriveBits(
      {
        ...
        info: enc.encode(`vaultpass:v1:${label}`),
      },
      key,
      256,
    ),
...
export async function derive(master: string, salt: string) {
  ...
  try {
    return {
      wrap: await hkdf(root, "wrap"),
      auth: hex(await hkdf(root, "auth")),
    };
  } finally {
    root.fill(0);
  }
...
export async function open(key: Uint8Array, envelope: Envelope, context: string): Promise<Uint8Array> {
  ...
  return new Uint8Array(
    await crypto.subtle.decrypt(
      {
        name: "AES-GCM",
        iv: buffer(unb64(envelope.nonce)),
        additionalData: enc.encode(context),
        tagLength: 128,
      },
      k,
...
export async function createAccount(master: string) {
  ...
  const id = crypto.randomUUID(),
    ...
    salt = hex(bytes(16));
  const derived = await derive(master, salt),
    accountKey = bytes(32),
    vaultKey = bytes(32);
  try {
    const bundle: Bundle = {
      salt,
      profile: PROFILE,
      account_key: await seal(derived.wrap, accountKey, context("account", id)),
    };
    return {
      id,
      vaultId,
      ...
      accountKey,
      vaultKey,
      wrappedVaultKey: await seal(accountKey, vaultKey, context("vault", id, vaultId)),
    };
  } finally {
    derived.wrap.fill(0);
  }
...
export async function unlock(master: string, id: string, bundle: Bundle) {
  ...
  try {
    return await open(d.wrap, bundle.account_key, context("account", id));
  } finally {
    d.wrap.fill(0);
  }
}
  ...
  try {
    return {
      ...
      bundle: {
        salt,
        profile: PROFILE,
        account_key: await seal(d.wrap, accountKey, context("account", id)),
      },
    };
  } finally {
    d.wrap.fill(0);
...
function recoveryBytes(value: string) {
  const normalized = value.trim().replace(/^VP1-/i, "").replaceAll(" ", "");
  const result = unhex(normalized);
  if (result.length !== 32) throw new Error("Recovery keys contain 64 hexadecimal characters");
  return result;
}
export async function createRecovery(accountKey: Uint8Array, id: string) {
  const root = bytes(32);
  try {
    const wrap = await hkdf(root, "recovery-wrap"),
      auth = await hkdf(root, "recovery-auth");
    try {
      return {
        recoveryKey: `VP1-${hex(root)}`,
        recoveryAuth: hex(auth),
        accountKey: await seal(wrap, accountKey, context("recovery", id)),
      };
    } finally {
      wrap.fill(0);
      auth.fill(0);
