...
The backend can verify completeness, authorization and freshness but cannot prove that opaque client submissions contain the correct key or plaintext. Already copied old plaintext, ciphertext and keys cannot be revoked. Owner transfer and owner-only deletion APIs are implemented without changing the team key because membership confidentiality is unchanged by transfer. The web management surface can create/open team vaults, edit encrypted items, manage invitations/roles, and perform the existing staged member-removal rotation entirely client-side. A safe self-leave workflow that preserves removal-rekey authorization guarantees, broader browser E2E/accessibility review, and mobile support remain unimplemented.
