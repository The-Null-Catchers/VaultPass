...
Web: hash-wasm Argon2id and browser Web Crypto. Mobile: PointyCastle Argon2id and `cryptography` AES-GCM/HKDF/HMAC. Backend: argon2-cffi for proof hashing. Library use does not establish that this integration is audited. Cross-client vectors, tamper tests, official TOTP vectors, and authorization tests are necessary but insufficient for production assurance.
