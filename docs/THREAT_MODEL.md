...
| Credential stuffing/brute force | Expensive derivation, proof hash, Redis limits; optional passkey user verification before session issuance | Accounts without passkeys and distributed/account-targeted throttling need additional hardening |
...
| Lost/passkey authenticator | Multiple passkeys supported; recovery removes lost credentials and revokes sessions | Without another authenticator or the one-time recovery key, the account and vault cannot be recovered |
