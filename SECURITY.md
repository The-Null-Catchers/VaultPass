...
The server stores client ciphertext and authentication/session metadata. Argon2id derives separated wrapping/authentication material; random account/vault keys are wrapped with AES-256-GCM. Optional passkey MFA uses WebAuthn user verification after the master-password proof and before session issuance. Optional recovery uses a client-generated 256-bit key, separated wrap/authentication material, and a one-time five-minute reset token. Sharing uses recipient RSA-OAEP key wrapping. See [architecture](docs/SECURITY_ARCHITECTURE.md), [threat model](docs/THREAT_MODEL.md), and [known limitations](docs/RELEASE_READINESS.md).
